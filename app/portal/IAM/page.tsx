...
export default function IamDashboardPage() {
  ...
  useEffect(() => {
    // 1. History stack ko multiple dummy levels se initialize karo taake back usi context me block ho
    window.history.pushState(null, '', window.location.href);
    window.history.pushState(null, '', window.location.href);

    const handlePopState = (event: PopStateEvent) => {
      // 2. Back event trigger hone par state dobara re-push karein bina pure page ko reload kiye
      window.history.pushState(null, '', window.location.href);
      
      // 3. Browser navigation engine ko safely runtime par push forward karo
      window.history.forward();
    };

    window.addEventListener('popstate', handlePopState);

    // Initial strict constraint execute karein
    window.history.forward();
