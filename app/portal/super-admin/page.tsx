...
export default function SuperAdminDashboardPage() {
  ...
  useEffect(() => {
    // 1. History stack ko advance dummy entries se fill karo bina navigation badle
    window.history.pushState(null, '', window.location.href);
    window.history.pushState(null, '', window.location.href);

    const handlePopState = (event: PopStateEvent) => {
      // 2. Back event dabe tou bina reload ke state instantly lock karo
      window.history.pushState(null, '', window.location.href);
      
      // 3. Browser ko dynamic forward check entry par priority do
      window.history.forward();
    };

    window.addEventListener('popstate', handlePopState);

    // Strict boundary enforcement on mount
    window.history.forward();
  ...
  useEffect(() => {
    ...
    if (storedName || storedEmail || storedRole || storedDept) {
      setProfile({
        ...
        email: storedEmail || 'system.root@ticketflow.internal',
        role: storedRole || 'SUPER_ADMIN',
        department: storedDept || 'Executive Operations',
      });
    }
  }, []);
